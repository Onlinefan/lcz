...
class ProductionPlan extends Model
{
    ...
    public static function createRecords($arProductionPlan, $arProductionFiles, $project)
    {
        foreach ($arProductionPlan['month'] as $key => $month) {
            ...
            $fileStart->createFile($arProductionFiles['preliminary_calculation_equipment'][$key], public_path('/Projects_files/' . $project->code . '/Управление проектом/Предварительный расчет оборудования/'), $fileStartName);

            $fileEnd = new File();
            $fileEndName = File::createName($project->name);
            if (isset($arProductionFiles['final_equipment_calculation'][$key]))
            $fileEnd->createFile($arProductionFiles['final_equipment_calculation'][$key], public_path('/Projects_files/' . $project->code . '/Управление проектом/Окончательный расчет оборудования/'), $fileEndName);
    ...
    public static function updateRecords($arProductionPlan, $arProductionFiles, $project)
    {
        ...
        foreach ($oldProductionPlan as $plan) {
            if (isset($plan->preliminaryCalculation)) {
                $fileSystem->delete(public_path('Projects_files/' . $project->code . '/Управление проектом/Предварительный расчет оборудования/' . $plan->preliminaryCalculation->file_name));
                $plan->preliminaryCalculation->delete();
            }

            if (isset($plan->finalCalculation)) {
                $fileSystem->delete(public_path('Projects_files/' . $project->code . '/Управление проектом/Окончательный расчет оборудования/' . $plan->finalCalculation->file_name));
