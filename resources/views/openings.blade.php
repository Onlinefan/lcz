                            ...
                            <table class="table table-striped table-bordered table-hover dataTables-example">
                                ...
                                @foreach ($projects as $project)
                                    <tr>
                                        ...
                                        <td>@if (isset($project->contract->lopFile))<span class="hidden-url">http://{{$_SERVER['SERVER_NAME'] . '/download?path=' . substr($project->contract->lopFile->path, strripos($project->contract->lopFile->path, 'Проекты/'))}}</span><a href="/download?path={{substr($project->contract->lopFile->path, strripos($project->contract->lopFile->path, 'Проекты/')) . $project->contract->lopFile->file_name}}">{{$project->contract->lopFile->file_name}}</a>@endif</td>
                                        <td>@if (isset($project->contract->lppListFile))<span class="hidden-url">http://{{$_SERVER['SERVER_NAME'] . '/download?path=' . substr($project->contract->lppListFile->path, strripos($project->contract->lppListFile->path, 'Проекты/'))}}</span><a href="/download?path={{substr($project->contract->lppListFile->path, strripos($project->contract->lppListFile->path, 'Проекты/')) . $project->contract->lppListFile->file_name}}">{{$project->contract->lppListFile->file_name}}</a>@endif</td>
                                        <td><a href="{{$project->contract->purchase_reference}}" target="_blank">{{$project->contract->purchase_reference}}</a></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
