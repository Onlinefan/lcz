                            ...
                            <table class="table table-striped table-bordered table-hover dataTables-example">
                                ...
                                @foreach ($productionPlan as $plan)
                                    <tr>
                                        ...
                                        <td>{{$plan->priority}}</td>
                                        <td>@if (isset($plan->preliminaryCalculation))<span class="hidden-url">http://{{$_SERVER['SERVER_NAME'] . '/download?path=' . substr($plan->preliminaryCalculation->path, strripos($plan->preliminaryCalculation->path, 'Проекты/'))}}</span><a href="/download?path={{substr($plan->preliminaryCalculation->path, strripos($plan->preliminaryCalculation->path, 'Проекты/')) . $plan->preliminaryCalculation->file_name}}">{{$plan->preliminaryCalculation->file_name}}</a>@endif</td>
                                        <td>@if (isset($plan->finalCalculation))<span class="hidden-url">http://{{$_SERVER['SERVER_NAME'] . '/download?path=' . substr($plan->finalCalculation->path, strripos($plan->finalCalculation->path, 'Проекты/'))}}</span><a href="/download?path={{substr($plan->finalCalculation->path, strripos($plan->finalCalculation->path, 'Проекты/')) . $plan->finalCalculation->file_name}}">{{$plan->finalCalculation->file_name}}</a>@endif</td>
